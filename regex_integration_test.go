...
func TestE2E_BuildIDRegexLabels(t *testing.T) {
	...
	// Create build status
	mr.Set(buildID+":queue:status", "ready")

	// Set build metrics
	mr.Lpush(buildID+":queue:unprocessed", "job1")
	mr.Lpush(buildID+":queue:unprocessed", "job2")
	mr.Lpush(buildID+":queue:unprocessed", "job3")
	mr.Set(buildID+":example_count", "100")
...
func TestE2E_BuildIDRegexMultipleBuilds(t *testing.T) {
	...
	for _, buildID := range builds {
		mr.Set(buildID+":queue:status", "ready")
		mr.Set(buildID+":example_count", "50")
	}
